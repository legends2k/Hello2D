...
void Sample2DSceneRenderer::Render()
{
	...
	context->DrawGeometry(m_pathLeftMountain.Get(), m_strokeBrush.Get());
	context->EndDraw();
}

void Sample2DSceneRenderer::CreateDeviceDependentResources()
{
	DX::ThrowIfFailed(m_deviceResources->GetD2DDeviceContext()->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Coral), &m_strokeBrush));
	MakeScenery();
...
{
	DX::ThrowIfFailed(m_deviceResources->GetD2DFactory()->CreatePathGeometry(&m_pathLeftMountain));
